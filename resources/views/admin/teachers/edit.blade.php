...
<title>แก้ไขผู้สอน</title><script src="https://cdn.tailwindcss.com"></script></head>
...
<header class="bg-sky-300 border-b border-sky-400 shadow-sm">
  <div class="max-w-6xl mx-auto h-16 px-4 md:px-6 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <h1 class="text-lg md:text-xl font-extrabold text-sky-950">แก้ไขผู้สอน</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl border p-5">
      <form id="teacherForm" action="{{ $updateUrl }}" method="POST" enctype="multipart/form-data">@csrf @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div><label class="{{ $label }}">ชื่อ *</label><input name="first_name" value="{{ old('first_name',$teacher->first_name) }}" class="{{ $input }}" required></div>
          <div><label class="{{ $label }}">นามสกุล *</label><input name="last_name"  value="{{ old('last_name',$teacher->last_name)   }}" class="{{ $input }}" required></div>
          ...
          <div><label class="{{ $label }}">วิชาหลัก</label><input name="primary_subject" value="{{ old('primary_subject',$teacher->primary_subject) }}" class="{{ $input }}"></div>
